...
df = df.fillna(0.0)
cutoff = 2.0
#df = df[ (df.ipo323/df.hog1 > cutoff)|(df.hog1/df.ipo323 > cutoff)
#  & (df.ipo323/df.myco5 > cutoff)|(df.myco5/df.ipo323 > cutoff)
#  & (df.ipo323/df.myco56 > cutoff)|(df.myco56/df.ipo323 > cutoff)]
tf.shade(ds.Canvas().points(df,'ipo323','myco5'))
...
x = Select(title='X-Axis: condition A', value='ipo323', options=sorted(list(df.iloc[:,2:].columns)))
x.on_change('value', update_data)

y = Select(title='Y-Axis: condition B', value='myco5', options=sorted(list(df.iloc[:,2:].columns)))
...
curdoc().title = "DE Gene Analysis – Z.tritici strains"